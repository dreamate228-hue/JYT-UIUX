...
async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "GET" && pathname.startsWith("/api/html-pages/")) {
      ...
      if (item?.url) {
        const html = await fetchBuffer(item.url);
      ...
      if (fs.existsSync(localFile)) return send(res, 200, await fs.promises.readFile(localFile), {
        "Content-Type": "text/html;charset=utf-8",
        "Content-Disposition": "inline"
      });
    ...
    if (req.method === "GET" && pathname === "/api/pdf") {
      ...
      const meta = await readPdfMeta();
      if (meta.files?.[kind]) {
        return send(res, 200, await fetchBuffer(meta.files[kind]), {
          "Content-Type": "application/pdf",
          "Content-Disposition": attachmentName(pdfFileName(kind)),
          "Cache-Control": "no-store"
        });
      }
      const fallback = pdfFallback(kind);
      if (!fallback || !fs.existsSync(fallback)) return send(res, 404, "not found");
      return send(res, 200, await fs.promises.readFile(fallback), {
        "Content-Type": "application/pdf",
        "Content-Disposition": attachmentName(pdfFileName(kind)),
        "Cache-Control": "no-store"
      });
